...
function MediaPage() {
  ...
  useEffect(() => {
    let dead = false;
    setMedia(null); setExtra(null); setSimilar([]); setCast([]); setProviders([]); setSeasons([]); setEpisodes([]); setError(null); setWl(getWatchlist());
    const cached = loadStashedMedia(mediaId);
    const load = cached && cached.type === mediaType ? Promise.resolve(cached) : fetchMediaById(mediaId, mediaType);
    load.then((m) => {
      if (dead) return;
      setMedia(m);
      stashWatchMedia(m);
      setCast(m.cast ?? []);
      fetchSimilar(m).then((items) => !dead && setSimilar(items)).catch(() => {});
      fetchCredits(m).then((credits) => !dead && credits.length && setCast(credits)).catch(() => {});
      fetchWatchProviders(m.id, m.type).then((p) => !dead && setProviders(p)).catch(() => {});
      fetchExtraDetails(m.id, m.type).then((d) => !dead && setExtra(d)).catch(() => {});
      if (m.type === "tv" || m.type === "anime") fetchTvDetails(m.id).then((d) => !dead && setSeasons(d.seasons)).catch(() => {});
  ...
  useEffect(() => {
    ...
    setEpisodesShown(3);
    fetchTvSeasonEpisodes(media.id, season).then(setEpisodes).catch(() => setEpisodes([]));
  }, [media, season]);

  const goBack = () => {
    if (typeof window === "undefined") { navigate({ to: "/" }); return; }
  ...
  if (!media) {
    return (
      <main className="grid min-h-screen place-items-center px-6 text-center">
        ...
        <div>
          <div className="text-xs uppercase tracking-[0.4em] text-primary/70">{error ? "Error" : "Loading"}</div>
    ...
    <main className="min-h-screen pb-16">
        ...
        <div className="relative mx-auto flex min-h-[48vh] max-w-7xl flex-col justify-end px-6 pb-2 pt-28 md:px-10 md:pb-3 md:pt-24">
          <div className="max-w-3xl space-y-6">
            ...
            <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-sm font-semibold text-foreground/75">
              ...
              {media.runtime && <><span className="h-1 w-1 rounded-full bg-foreground/30" /><span>{media.runtime}</span></>}
              <span className="h-1 w-1 rounded-full bg-foreground/30" />
              <span className="inline-flex items-center gap-1.5 text-foreground">
                <svg viewBox="0 0 16 16" className="h-3.5 w-3.5 text-primary" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><circle cx="8" cy="8" r="5.2"/><path d="M8 4.8v3.4l2.4 1.4"/></svg>
                {media.rating.toFixed(1)}
              </span>
              {extra?.contentRating && (
                <span className="rounded border border-white/25 px-1.5 py-0.5 text-[10px] uppercase tracking-wider">{extra.contentRating}</span>
              )}
            ...
            <div className="flex flex-wrap items-center gap-2 pt-2 pb-1">
              ...
              >
                <svg viewBox="0 0 24 24" className="h-4 w-4 fill-current transition-transform group-hover/btn:scale-110"><path d="M8 5v14l11-7z" /></svg>
              ...
              >
                {inWl ? (
                  <svg viewBox="0 0 24 24" className="h-[18px] w-[18px]" fill="none" stroke="currentColor" strokeWidth="2.4"><path d="M5 12l5 5L20 7"/></svg>
                ) : (
                  <svg viewBox="0 0 24 24" className="h-[18px] w-[18px]" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 5v14m-7-7h14"/></svg>
                )}
              </button>

              {extra?.trailerKey && (
                <IconBtn onClick={() => setTrailerOpen(true)} label="Trailer">
                  <svg viewBox="0 0 24 24" className="h-[18px] w-[18px]" fill="none" stroke="currentColor" strokeWidth="2"><polygon points="23 7 16 12 23 17 23 7"/><rect x="1" y="5" width="15" height="14" rx="2" ry="2"/></svg>
                </IconBtn>
              )}
              <IconBtn onClick={handleWatchParty} label="Watch Party">
                <svg viewBox="0 0 24 24" className="h-[18px] w-[18px]" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="9" cy="8" r="3"/><circle cx="17" cy="9" r="2.5"/><path d="M3 19c0-3 3-5 6-5s6 2 6 5"/><path d="M14 19c0-2 2-4 4-4s4 2 4 4"/></svg>
              </IconBtn>
              <IconBtn onClick={handleShare} label="Share">
                <svg viewBox="0 0 24 24" className="h-[18px] w-[18px]" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/><line x1="8.59" y1="13.51" x2="15.42" y2="17.49"/><line x1="15.41" y1="6.51" x2="8.59" y2="10.49"/></svg>
              </IconBtn>
            ...
            <div className="animate-soft-rise">
              <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">Season {season}{episodes.length ? ` · ${episodes.length} episodes` : ""}</p>
                </div>
                {seasons.length > 0 && (
                  <SeasonPicker seasons={seasons} value={season} onChange={(s) => { setSeason(s); setEpisode(1); }} />
                )}
              </div>
              <div className="space-y-2.5">
                ...
                {episodes.length > episodesShown && (
                  ...
                  >
                    Load {Math.min(6, episodes.length - episodesShown)} more · {episodes.length - episodesShown} remaining
                  </button>
                )}
          ...
          {similar.length > 0 && (
            <div>
              ...
              <div className="no-scrollbar -mx-2 flex gap-4 overflow-x-auto px-2 pb-3 pt-3">
                {similar.map((m) => <MediaCard key={`${m.type}-${m.id}`} media={m} />)}
              </div>
            </div>
          )}
        </div>
        ...
        <aside className="h-fit space-y-6 rounded-3xl border border-white/10 bg-white/[0.03] p-6 backdrop-blur-sm animate-soft-rise md:sticky md:top-6">
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-xs font-medium text-muted-foreground">{isSeries ? "Series" : "Movie"} · {media.year}</p>
            </div>
          ...
          <div>
            <h3 className="text-[10px] font-semibold uppercase tracking-[0.28em] text-muted-foreground">Where to watch</h3>
            {providers.length > 0 ? (
                ...
                {providers.map((p) => (
                  ...
                  >
                    ...
                    <span className="min-w-0">
                      ...
                      <span className="mt-0.5 block text-[10px] font-semibold uppercase tracking-[0.14em] text-muted-foreground">{p.access || "Watch"}</span>
                    </span>
                  </a>
                ))}
              </div>
          ...
          {extra?.spokenLanguages?.length ? <InfoBlock title="Languages" items={extra.spokenLanguages} /> : null}
          {extra?.productionCountries?.length ? <InfoBlock title="Countries" items={extra.productionCountries} /> : null}
          {extra?.createdBy?.length ? <InfoBlock title="Created by" items={extra.createdBy} /> : null}
          ...
          <div className="space-y-2 border-t border-white/10 pt-5 text-sm">
            ...
            {isSeries ? (
              <>
                ...
                {extra?.numberOfSeasons ? <Detail k="Seasons" v={String(extra.numberOfSeasons)} /> : null}
                {extra?.numberOfEpisodes ? <Detail k="Episodes" v={String(extra.numberOfEpisodes)} /> : null}
              </>
            ) : (
              <>
      ...
      {trailerOpen && extra?.trailerKey && (
        ...
        >
          ...
          >
            <svg viewBox="0 0 24 24" className="h-5 w-5" fill="none" stroke="currentColor" strokeWidth="2.2"><path d="M6 6l12 12M18 6 6 18" strokeLinecap="round"/></svg>
          </button>
          <div
...
function SeasonPicker({ seasons, value, onChange }: { seasons: { number: number }[]; value: number; onChange: (n: number) => void }) {
  const [open, setOpen] = useState(false);
  if (seasons.length <= 1) {
    return <span className="rounded-full bg-white/5 px-3 py-1.5 text-xs font-medium text-muted-foreground ring-1 ring-white/10">Season {value}</span>;
  }
  return (
    <div className="relative">
      ...
      >
        ...
        <svg viewBox="0 0 24 24" className={`h-3.5 w-3.5 transition-transform ${open ? "rotate-180" : ""}`} fill="none" stroke="currentColor" strokeWidth="2.4"><path d="m6 9 6 6 6-6" strokeLinecap="round" strokeLinejoin="round"/></svg>
      </button>
      {open && (
        <>
          ...
          <div className="absolute right-0 top-full z-50 mt-2 max-h-72 w-44 overflow-y-auto rounded-2xl border border-white/10 bg-popover/95 p-1.5 shadow-2xl backdrop-blur-xl animate-modal-in">
            {seasons.map((s) => (
              ...
              >
                ...
                {s.number === value && <svg viewBox="0 0 24 24" className="h-3.5 w-3.5 text-primary" fill="none" stroke="currentColor" strokeWidth="2.6"><path d="M5 12l5 5L20 7" strokeLinecap="round" strokeLinejoin="round"/></svg>}
              </button>
            ))}
          </div>
        </>
...
function BackButton({ onClick }: { onClick: () => void }) {
  return (
    ...
    >
      <svg viewBox="0 0 24 24" className="h-5 w-5" fill="none" stroke="currentColor" strokeWidth="2.3"><path d="M15 18 9 12l6-6" strokeLinecap="round" strokeLinejoin="round" /></svg>
    </button>
  );
}
...
function PeopleStrip({ title, people }: { title: string; people: { id?: number; name: string; role: string; img?: string }[] }) {
  return (
    <div>
      ...
      <div className="no-scrollbar -mx-2 flex gap-4 overflow-x-auto px-2 pb-3 pt-2">
        {people.slice(0, 18).map((p) =>
          p.id ? (
            <Link key={`${p.id}-${p.role}`} to="/person/$id" params={{ id: String(p.id) }} className="group w-32 shrink-0 text-center">
              <PersonTile person={p} />
            </Link>
          ) : (
  ...
  return (
    <>
      <div className="aspect-square overflow-hidden rounded-2xl bg-white/5 ring-1 ring-white/10 transition group-hover:ring-primary/45">
        {person.img ? (
          <img src={person.img} alt={person.name} className="h-full w-full object-cover transition group-hover:scale-[1.04]" />
        ) : (
          <div className="grid h-full place-items-center text-3xl text-white/30">{person.name[0]}</div>
        )}
      </div>
...
function WhereToWatchFallback({ media, extra }: { media: Media; extra: ExtraDetails | null }) {
  ...
  const daysSince = releaseDate ? Math.floor((today.getTime() - releaseDate.getTime()) / 86_400_000) : null;
  ...
  if (media.type === "movie") {
    if (isFuture || status === "Post Production" || status === "In Production" || status === "Planned") {
      label = "Coming soon";
  ...
  return (
    <div className={`mt-3 flex items-center gap-3 rounded-2xl bg-gradient-to-br ${accent} p-3 ring-1`}>
      <div className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-black/30 ring-1 ring-white/10">
        {tone === "theatre" ? (
          <svg viewBox="0 0 24 24" className="h-5 w-5" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M3 7h18M3 12h18M3 17h18"/><path d="M7 3v18M17 3v18"/></svg>
        ) : tone === "soon" ? (
          <svg viewBox="0 0 24 24" className="h-5 w-5" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="9"/><path d="M12 7v5l3 2"/></svg>
        ) : (
          <svg viewBox="0 0 24 24" className="h-5 w-5" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="5" width="18" height="14" rx="2"/><path d="M3 10h18"/></svg>
        )}
      </div>
  ...
  return (
    <div>
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        {items.map((item) => (
          <span key={item} className="rounded-full bg-white/5 px-3 py-1 text-xs ring-1 ring-white/10">{item}</span>
        ))}
      </div>
    </div>
  );
