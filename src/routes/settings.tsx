...
function Slider({ value, onChange, min = 0, max = 100, step = 1, suffix }: { value: number; onChange: (v: number) => void; min?: number; max?: number; step?: number; suffix?: string }) {
  return (
    <div className="flex w-64 items-center gap-3">
      <input
        ...
        style={{ background: `linear-gradient(to right, var(--primary) 0%, var(--primary) ${((value - min) / (max - min)) * 100}%, color-mix(in oklab, var(--foreground) 10%, transparent) ${((value - min) / (max - min)) * 100}%, color-mix(in oklab, var(--foreground) 10%, transparent) 100%)` }}
      />
    ...
    <div ref={ref} className="relative min-w-[10rem]">
      ...
      <div className={`absolute right-0 top-12 z-40 w-52 overflow-hidden rounded-2xl border border-white/10 bg-[oklch(0.16_0.02_280)] p-1.5 text-white shadow-2xl transition duration-150 ${open ? "translate-y-0 opacity-100" : "pointer-events-none -translate-y-1 opacity-0"}`}>
        {options.map((o) => (
          ...
          >
            {o.label}
            {value === o.value && <svg viewBox="0 0 24 24" className="h-3.5 w-3.5 text-primary" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M5 12l5 5L20 7"/></svg>}
          </button>
        ))}
      </div>
    </div>
...
function IntegrationCard({ name, desc, placeholder, value, onChange }: { name: string; desc: string; placeholder: string; value: string; onChange: (v: string) => void }) {
  ...
  return (
    <div className="rounded-2xl border border-glass-border bg-background/30 p-4 transition hover:border-primary/40">
      <div className="flex items-start justify-between gap-3">
        ...
        {connected && <button onClick={() => onChange("")} className="text-xs text-muted-foreground hover:text-destructive">Clear</button>}
      </div>
      <div className="mt-3"><TextField value={value} onChange={onChange} placeholder={placeholder} type="password" /></div>
    </div>
  );
}
      ...
      <div className="mx-auto max-w-4xl space-y-6">
        ...
        >
          <div className="space-y-3">
            ...
            <p className="text-[11px] text-muted-foreground">
              Tip: in your browser DevTools → Application → Cookies for febbox.com, copy the value of the <code className="rounded bg-white/5 px-1">ui</code> cookie and paste it here.
            </p>
          </div>
        </Section>
        ...
        >
          ...
          <Row label="Detect now" hint="Refresh the IP-based region cache.">
            <RegionDetectButton onPicked={(r) => setInt({ pstreamRegion: r === "unknown" ? "auto" : (r as Settings["integrations"]["pstreamRegion"]) })} />
          </Row>
        </Section>
        ...
        <Section title="Integrations" desc="Paste an API key to connect — a green check confirms it's saved.">
          <div className="grid gap-3 md:grid-cols-2">
            <IntegrationCard name="FebBox Cookie" desc="Primary direct source. Paste your FebBox ui= cookie to unlock direct HLS streams up to 4K. Without it, the player uses embed backups." placeholder="ui=… cookie" value={ints.febboxCookie} onChange={(v) => setInt({ febboxCookie: v })} />
            
            <IntegrationCard name="Real-Debrid" desc="Premium high-speed links from hosters." placeholder="Real-Debrid API token" value={ints.realDebrid} onChange={(v) => setInt({ realDebrid: v })} />
            <IntegrationCard name="AllDebrid" desc="Alternative debrid network." placeholder="AllDebrid API key" value={ints.allDebrid} onChange={(v) => setInt({ allDebrid: v })} />
            <IntegrationCard name="Premiumize" desc="Cloud download + streaming." placeholder="Premiumize API key" value={ints.premiumize} onChange={(v) => setInt({ premiumize: v })} />
            <IntegrationCard name="Trakt.tv" desc="Sync your watch history and lists." placeholder="Trakt OAuth token" value={ints.traktToken} onChange={(v) => setInt({ traktToken: v })} />
            <IntegrationCard name="Simkl" desc="Track anime, TV and movies." placeholder="Simkl OAuth token" value={ints.simklToken} onChange={(v) => setInt({ simklToken: v })} />
            <IntegrationCard name="OpenSubtitles" desc="Multi-language subtitles." placeholder="OpenSubtitles API key" value={ints.openSubtitles} onChange={(v) => setInt({ openSubtitles: v })} />
          </div>
        </Section>

        <Section title="Catalog">
          <Row label="Custom TMDB API key" hint="Leave blank to use the built-in key.">
            <div className="w-72"><TextField value={s.tmdbApiKey} onChange={(v) => set({ tmdbApiKey: v })} placeholder="optional" /></div>
          </Row>
          <Row label="Region"><div className="w-24"><TextField value={s.region} onChange={(v) => set({ region: v })} /></div></Row>
          <Row label="Language">
            <Select value={s.language} onChange={(v) => set({ language: v as Settings["language"] })} options={[
              { value: "en", label: "English" }, { value: "es", label: "Español" }, { value: "fr", label: "Français" }, { value: "ja", label: "日本語" }, { value: "de", label: "Deutsch" },
            ]} />
          </Row>
        ...
        <button
          onClick={() => { localStorage.removeItem("sleepy.settings.v2"); location.reload(); }}
