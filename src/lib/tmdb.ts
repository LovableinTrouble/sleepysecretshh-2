...
export function isMatureAllowed(): boolean {
  ...
  try { return Boolean(getSettings()?.matureContent); } catch { return false; }
}
...
async function tmdb<T = any>(path: string, params: Record<string, string | number | boolean> = {}): Promise<T> {
  const u = new URL(BASE + path);
  u.searchParams.set("api_key", key());
  u.searchParams.set("language", "en-US");
  // Always force adult OFF at the request layer unless the user explicitly opted in.
  if (!isMatureAllowed() && !("include_adult" in params)) u.searchParams.set("include_adult", "false");
...
function toMedia(raw: any, kind: MediaKind, genres: Record<number, string>): Media {
  const isMovie = kind === "movie" || (kind !== "tv" && kind !== "anime" && raw.media_type === "movie");
  ...
  return {
    ...
    genres: ids.map((id) => genres[id]).filter(Boolean).slice(0, 4),
    studios: (raw.production_companies || raw.networks || []).map((c: any) => c.name).filter(Boolean).slice(0, 4),
...
export function rememberOne(m: Media) { SEEN.set(m.id, m); return m; }
export function getRemembered(id: number) { return SEEN.get(id); }
...
export async function fetchExtraDetails(id: number, kind: MediaKind): Promise<ExtraDetails> {
  ...
  return {
    ...
    spokenLanguages: (raw.spoken_languages || []).map((l: any) => l.english_name || l.name).filter(Boolean),
...
export async function fetchTrending(kind: "movie" | "tv" | "all" = "all"): Promise<Media[]> {
  ...
  return remember(d.results.filter((r) => r.poster_path && isReleased(r) && isSafeForMode(r)).map((r) => {
...
export async function fetchPopular(kind: "movie" | "tv", pages = 3): Promise<Media[]> {
  ...
  return remember(all.filter((r) => r.poster_path && isReleased(r) && isSafeForMode(r)).map((r) => toMedia(r, kind, g)));
}
...
export async function fetchByGenre(kind: "movie" | "tv", genreId: number, pages = 1): Promise<Media[]> {
  const g = await genreMap(kind);
  const reqs = Array.from({ length: pages }, (_, i) =>
...
export async function fetchNowPlaying(): Promise<Media[]> {
  ...
  return remember(d.results.filter((r) => r.poster_path && isReleased(r) && isSafeForMode(r)).map((r) => toMedia(r, "movie", g)));
}
export async function fetchUpcoming(pages = 2): Promise<Media[]> {
  ...
  return remember(all.filter((r) => r.poster_path && !isReleased(r)).map((r) => toMedia(r, "movie", g)));
}
export async function fetchAiringToday(): Promise<Media[]> {
  ...
  return remember(d.results.filter((r) => r.poster_path && isReleased(r) && isSafeForMode(r)).map((r) => toMedia(r, "tv", g)));
}
    ...
    Promise.all(
      Array.from({ length: pages }, (_, i) =>
        tmdb<{ results: any[] }>(`/discover/tv`, {
          ...
          sort_by: "popularity.desc",
          "air_date.gte": new Date(Date.now() - 1000 * 60 * 60 * 24 * 120).toISOString().slice(0, 10),
        }),
      ),
    ),
  ]);
  const trendingAnime = trendingPages.flatMap((r) => r.results).filter(
...
export async function fetchAnimeByGenre(extraGenreId: number, pages = 1): Promise<Media[]> {
  ...
  const all = (await Promise.all(reqs)).flatMap((r) => r.results);
  return remember(all.filter((r) => r.poster_path && isReleased(r) && isSafeForMode(r)).map((r) => toMedia(r, "anime", g)));
}
...
export async function fetchSimilar(media: Media): Promise<Media[]> {
  ...
  const collect = (items: any[]) => items.filter((r) => r.poster_path && isReleased(r) && isSafeForMode(r));
  const [similar, recommendations] = await Promise.all([
    tmdb<{ results: any[] }>(`/${tmdbKind}/${media.id}/similar`).catch(() => ({ results: [] })),
    tmdb<{ results: any[] }>(`/${tmdbKind}/${media.id}/recommendations`).catch(() => ({ results: [] })),
  ]);
  let results = collect([...similar.results, ...recommendations.results]);
  if (results.length < 6) {
    const genreIds = media.genres.map((name) => Number(Object.entries(g).find(([, value]) => value === name)?.[0])).filter(Boolean);
    if (genreIds.length) {
...
export async function fetchCredits(media: Media): Promise<{ id: number; name: string; role: string; img?: string }[]> {
  const tmdbKind = media.type === "movie" ? "movie" : "tv";
...
export async function fetchCrew(media: Media): Promise<{ id: number; name: string; role: string; img?: string }[]> {
  const tmdbKind = media.type === "movie" ? "movie" : "tv";
  ...
  const composers = crew.filter((p) => p.job === "Original Music Composer" || p.job === "Music").slice(0, 2);
  ...
  const all = [...directors, ...writers, ...producers, ...composers, ...cinematographers, ...editors];
  return all
...
export async function fetchWatchProviders(id: number, kind: MediaKind): Promise<WatchProvider[]> {
  ...
  const d = await tmdb<{ results: Record<string, any> }>(`/${tmdbKind}/${id}/watch/providers`).catch(() => ({ results: {} as Record<string, any> }));
  const us = d.results.US;
  const groups: Array<["Stream" | "Free" | "Rent" | "Buy", any[]]> = [
    ["Stream", us?.flatrate || []],
    ["Free", us?.free || us?.ads || []],
    ["Rent", us?.rent || []],
    ["Buy", us?.buy || []],
  ...
  const seen = new Set<number>();
  return groups.flatMap(([access, items]) =>
    items.map((p: any) => ({
      id: p.provider_id,
...
export async function fetchTvDetails(tvId: number): Promise<{ seasons: { number: number }[] }> {
  ...
  return { seasons: (d.seasons || []).filter((s: any) => s.season_number > 0).map((s: any) => ({ number: s.season_number })) };
}
...
export const STREAMING_SERVICES: StreamingService[] = [
  { id: 8,    name: "Netflix",         slug: "netflix",     logo: "https://image.tmdb.org/t/p/original/t2yyOv40HZeVlLjYsCsPHnWLk4W.jpg",  accent: "#E50914", blurb: "Originals, blockbusters, anime" },
  { id: 9,    name: "Prime Video",     slug: "prime-video", logo: "https://image.tmdb.org/t/p/original/emthp39XA2YScoYL1p0sdbAH2WA.jpg",  accent: "#00A8E1", blurb: "Amazon originals and hit films" },
  { id: 337,  name: "Disney+",         slug: "disney-plus", logo: "https://image.tmdb.org/t/p/original/97yvRBw1GzX7fXprcF80er19ot.jpg",   accent: "#0E2A56", blurb: "Disney, Pixar, Marvel, Star Wars" },
  { id: 1899, name: "Max",             slug: "max",         logo: "https://image.tmdb.org/t/p/original/jbe4gVSfRlbPTdESXhEKpornsfu.jpg",  accent: "#002BE7", blurb: "HBO prestige TV and films" },
  { id: 15,   name: "Hulu",            slug: "hulu",        logo: "https://image.tmdb.org/t/p/original/giwM8XX4V2AQb9vsoN7yti82tKK.jpg",  accent: "#1CE783", blurb: "Next-day TV, FX and originals" },
  { id: 350,  name: "Apple TV+",       slug: "apple-tv",    logo: "https://image.tmdb.org/t/p/original/peURlLlr8jggOwK53fJ5wdQl05y.jpg",  accent: "#111111", blurb: "Apple's flagship originals" },
  { id: 531,  name: "Paramount+",      slug: "paramount",   logo: "https://image.tmdb.org/t/p/original/h5DcR0J2EESLitnhR8xLG1QymTE.jpg",  accent: "#0064FF", blurb: "Star Trek, Yellowstone, CBS hits" },
  { id: 386,  name: "Peacock",         slug: "peacock",     logo: "https://image.tmdb.org/t/p/original/drPlq5beqXtBaP7MNs8W616YRhm.jpg",  accent: "#000000", blurb: "NBC shows and Universal films" },
  { id: 387,  name: "Peacock Premium", slug: "peacock-plus",logo: "https://image.tmdb.org/t/p/original/8VCV78prwd9QzZnEm0ReO6bERDa.jpg",  accent: "#000000", blurb: "Ad-free Peacock catalog" },
  { id: 2,    name: "Apple TV",        slug: "apple-tv-store", logo: "https://image.tmdb.org/t/p/original/peURlLlr8jggOwK53fJ5wdQl05y.jpg", accent: "#1d1d1f", blurb: "Buy / rent on the iTunes store" },
  { id: 192,  name: "YouTube",         slug: "youtube",     logo: "https://image.tmdb.org/t/p/original/qZE9Mp4dG2EOd2NLzWtINQs9YQt.jpg",  accent: "#FF0000", blurb: "Movies on YouTube" },
  { id: 257,  name: "fuboTV",          slug: "fubo",        logo: "https://image.tmdb.org/t/p/original/3IFsLzCxqyqzgmw3oRZdLQHbWFy.jpg",  accent: "#FA4615", blurb: "Live TV and on-demand" },
];
...
export async function fetchByProvider(providerId: number, pages = 2, region = "US"): Promise<Media[]> {
  ...
  const allMovies = movies.flatMap((r) => r.results).filter((r) => r.poster_path && isReleased(r) && isSafeForMode(r)).map((r) => toMedia(r, "movie", mg));
  const allTv = tv.flatMap((r) => r.results).filter((r) => r.poster_path && isReleased(r) && isSafeForMode(r)).map((r) => toMedia(r, "tv", tg));
